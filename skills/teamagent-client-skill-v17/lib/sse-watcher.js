...
let stopRequested = false  // agent:paused 后置 true，阻止重连
...
function scheduleReconnect(reason) {
  if (stopRequested) {
    console.log(`⏸️ Watch 已停止（paused），不再重连`)
    return
  }
  reconnectCount++
  let delay
...
/** event-handlers 调用：标记停止（收到 agent:paused 后不再重连） */
function requestStop(reason) {
  stopRequested = true
  sseConnected = false
  console.log(`⏸️ [requestStop] ${reason || '停止请求'}`)
}
...
// 30s 轮询兜底（SSE 静默断连时）—— 聊天补拉
function startChatPoll() {
  if (chatPollTimer) clearInterval(chatPollTimer)
  chatPollTimer = setInterval(async () => {
    if (sseConnected && Date.now() - lastSSEActivity < 60000) return
    if (!sseConnected) console.log('⚠️  [轮询] SSE 不活跃，主动拉取聊天...')
    await catchupUnreadChat(new Date(Date.now() - 120000).toISOString())
  }, 30000)
}

// 30s 轮询兜底 —— 步骤补拉（Lobster方案：SSE丢步骤时的保底网）
// 仅在 SSE 断线 或 SSE 超过 60s 静默时触发，SSE 活跃时零开销
function startStepPoll() {
  setInterval(async () => {
    const sseStale = !sseConnected || (Date.now() - lastSSEActivity > 60000)
    if (!sseStale) return  // SSE 活跃，不占资源

    if (catchupInProgress) return  // 补拉中，跳过
    ...
    try {
      ...
      const newSteps = steps.filter(s => !dedup.isDuplicate(`step-${s.id}`))
      if (newSteps.length > 0) {
        console.log(`🔄 [步骤轮询] SSE 不活跃，捞到 ${newSteps.length} 个待执行步骤`)
        await executor.autoPickupNextSteps(newSteps.length + 2)
      }
    } catch (e) {
      console.error('🔄 [步骤轮询] 失败:', e.message)
    }
  }, 30000)
}

// P2-4: 步骤超时检测 — 本地追踪 in_progress 步骤
const TIMEOUT_CHECK_INTERVAL = 5 * 60 * 1000   // 5 分钟检查一次
const STEP_TIMEOUT_MS = 30 * 60 * 1000          // 30 分钟超时
const inProgressSteps = new Map()                // stepId → { title, startTime }
const notifiedTimeouts = new Set()               // 已通知过的 stepId

/** 由 event-handlers / step-executor 调用：记录步骤开始执行 */
function trackStepStart(stepId, title) {
  inProgressSteps.set(stepId, { title: title || stepId, startTime: Date.now() })
}

/** 由 event-handlers / step-executor 调用：记录步骤完成 */
function trackStepDone(stepId) {
  inProgressSteps.delete(stepId)
  notifiedTimeouts.delete(stepId)
}

function startTimeoutCheck() {
  setInterval(async () => {
    const now = Date.now()
    for (const [stepId, info] of inProgressSteps) {
      const elapsed = now - info.startTime
      if (elapsed < STEP_TIMEOUT_MS) continue
      if (notifiedTimeouts.has(stepId)) continue

      const minutes = Math.round(elapsed / 60000)
      console.log(`\n⏰ [超时] 步骤「${info.title}」已执行 ${minutes} 分钟，可能卡住了`)

      try {
        await client.request('POST', '/api/chat/push', {
          content: `⏰ 步骤「${info.title}」已在 in_progress 状态超过 ${minutes} 分钟，可能需要人工介入。`
        })
      } catch (_) {}

      notifiedTimeouts.add(stepId)
    }
  }, TIMEOUT_CHECK_INTERVAL)
}

/** NEW-1: step:approved 后主动触发补拉（拉取下一可用步骤） */
function triggerHeartbeat() {
  console.log('💓 [心跳] step:approved 触发，主动补拉下一步骤')
  onReconnected()
}

module.exports = { init, connect, startChatPoll, startStepPoll, startTimeoutCheck, trackStepStart, trackStepDone, triggerHeartbeat, requestStop }
