...
- ✅ 已在 [TeamAgent 网站](https://agent.avatargaia.top) 注册账号
...
默认连接 `https://agent.avatargaia.top`。

配置文件位置：`~/.teamagent/config.json`
...
> 💡 Token 同时也是 LLM 代理网关的认证凭证。OpenClaw 配置 LLM 时使用 `baseURL: https://agent.avatargaia.top/api/llm/v1`，`apiKey` 填同一个 `ta_xxx` Token。
