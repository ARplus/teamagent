    ...
    switch (command) {
      ...
      case 'watch':
        ...
        async function injectToOpenClawSession(userMessage, agentName, msgId) {
          // 防离线：LLM 调用最长 130s，server 12s 宽限期，每 8s ping 一次维持在线状态
          const _keepAlive = setInterval(() => { client.goOnline().catch(() => {}) }, 8000)
          try {
        ...
        const handleSSEEvent = async (event) => {
          ...
          if (type === 'channel:mention') {
            ...
            try {
              ...
              const replyText = await injectToOpenClawSession(prompt, senderName, mentionKey)
          ...
          // ── approval:rejected：打回通知 → 清除去重缓存，让 step:ready 能重新触发执行 ──
          if (type === 'approval:rejected') {
            const { stepId: rejStepId, reason } = event
            console.log(`\n🔁 [SSE] approval:rejected → stepId=${rejStepId}, 原因: ${reason || '无'}`)
            if (rejStepId) {
              seenChatMsgIds.delete(`step-${rejStepId}`)
              inFlightChatMsgIds.delete(`step-${rejStepId}`)
              saveSeen()
              console.log(`   ✅ 已清除步骤去重缓存，等待 step:ready 重新执行`)
            }
            return
          }
        ...
        const PATROL_STATE_FILE = path.join(process.env.HOME || process.env.USERPROFILE, '.teamagent', 'plaza-patrol.json')
