    ...
    switch (command) {
      ...
      case 'exam-take': {
        ...
        try {
          ...
          console.log('\n💡 答案格式: [{questionId:"q1",answer:"A"}, {questionId:"q2",answer:["A","C"]}, ...]')
      ...
      case 'update': {
        try {
          ...
          if (isWin) {
            execSync(`powershell -Command "Expand-Archive -Path '${tmpZip}' -DestinationPath '${require('os').tmpdir()}/ta-skill-update' -Force"`)
            const extractedDir = path.join(require('os').tmpdir(), 'ta-skill-update', 'teamagent-client-skill')
            if (fs.existsSync(extractedDir)) {
              const files = fs.readdirSync(extractedDir)
              files.forEach(f => {
                fs.copyFileSync(path.join(extractedDir, f), path.join(__dirname, f))
              })
            }
            // 清理
            execSync(`powershell -Command "Remove-Item -Recurse -Force '${require('os').tmpdir()}/ta-skill-update'"`)
          } else {
            execSync(`unzip -o "${tmpZip}" -d "${require('os').tmpdir()}/ta-skill-update"`)
            const extractedDir = path.join(require('os').tmpdir(), 'ta-skill-update', 'teamagent-client-skill')
            if (fs.existsSync(extractedDir)) {
              execSync(`cp -f ${extractedDir}/* "${__dirname}/"`)
            }
            execSync(`rm -rf "${require('os').tmpdir()}/ta-skill-update"`)
          }
          // 清理 zip
