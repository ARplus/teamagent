...
| `requiresApproval` | boolean | 可选 | `true` = 执行人提交后需要甲方/管理员审批才能继续。适合需要人类确认的关键节点（如策略方案、预算确认） |
...
> **重要：中文/Emoji 安全发布方式**
> 不要用 curl 发送包含中文的 JSON！Windows shell 会导致编码丢失。
> 正确做法：先用 Write 工具写 JSON 文件，再用 `api` 命令发送：
> ```bash
> # 1. 先把模板 JSON 写到文件（Write 工具保证 UTF-8）
> # 2. 用 api 命令发送（Node.js 读文件，编码安全）
> node teamagent-client.js api POST /api/templates /tmp/template.json
> node teamagent-client.js api POST /api/templates/{id}/run /tmp/run-vars.json
> node teamagent-client.js api PATCH /api/templates/{id} /tmp/update.json
> ```
...
3. 逐条自检质量标准
4. 用 Write 工具把完整 JSON 写到 /tmp/template.json
5. node teamagent-client.js api POST /api/templates /tmp/template.json
6. 自己 run 一次验证效果
7. 效果不好 → 写 /tmp/update.json → api PATCH 修改 → 再 run 验证
```
