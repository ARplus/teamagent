...
let _clientRef = null  // watch 模式设置，退出时用

async function gracefulExit(signal) {
  clearPid()
  if (_clientRef) {
    try { await _clientRef.setStatus('offline') } catch (_) {}
  }
  process.exit(0)
}
...
async function main() {
  ...
  try {
    ...
    switch (command) {
      ...
      case 'watch':
        writePid()
        _clientRef = client  // 供 gracefulExit 使用
        console.log(`📡 开始 SSE 实时监控模式（PID=${process.pid}，Ctrl+C 退出）\n`)

        // 启动时重置状态为在线（清除上次残留的 working 状态）
        try { await client.goOnline() } catch (_) {}
        console.log('\u2705 \u72b6\u6001\u5df2\u8bbe\u4e3a\u5728\u7ebf')

        // \u6bcf 60s \u53d1\u4e00\u6b21\u5fc3\u8df3\uff0c\u670d\u52a1\u7aef 3min \u65e0\u5fc3\u8df3\u5219\u6807\u79bb\u7ebf
        const heartbeatTimer = setInterval(async () => {
          try { await client.request('POST', '/api/agent/heartbeat', {}) } catch (_) {}
        }, 60 * 1000)
        ...
        const handleSSEEvent = async (event) => {
          ...
          if (type === 'step:ready') {
            ...
            if (stepType === 'decompose') {
              ...
              // P0-1 fix: 人类步骤不自动执行，等人类自己来做
              if (event.assigneeType === 'human') {
                console.log('   👤 人类步骤，跳过自动执行')
                markSeen(stepKey)
                return
              }
              // 🔧 Agent/auto 步骤统一自动执行
