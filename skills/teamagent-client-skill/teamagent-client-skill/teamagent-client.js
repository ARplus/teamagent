...
if (require.main === module) {
  ...
  async function main() {
    ...
    switch (command) {
      ...
      // ========== 版本 & 自更新 ==========
      case 'version':
        console.log(`📦 TeamAgent Skill ${SKILL_VERSION}`)
        break

      case 'update': {
        console.log(`🔄 TeamAgent Skill 自更新 (当前: ${SKILL_VERSION})`)
        const updateUrl = 'https://agent.avatargaia.top/static/teamagent-client-skill.zip'
        const tmpZip = path.join(os.tmpdir(), 'teamagent-client-skill-update.zip')
        const skillDir = path.dirname(__dirname) // parent of teamagent-client-skill/

        console.log(`📥 下载最新包: ${updateUrl}`)
        await new Promise((resolve, reject) => {
          const zipFile = fs.createWriteStream(tmpZip)
        ...
        console.log(`📦 解压到 ${skillDir} ...`)
        const { execSync } = require('child_process')
        if (process.platform === 'win32') {
          execSync(`powershell -Command "Expand-Archive -Path '${tmpZip}' -DestinationPath '${skillDir}' -Force"`)
        } else {
          execSync(`unzip -o "${tmpZip}" -d "${skillDir}"`)
        }
        fs.unlinkSync(tmpZip)

        // 读取新版本号
        try {
          const newSkillMd = fs.readFileSync(path.join(skillDir, 'teamagent-client-skill', 'SKILL.md'), 'utf-8')
          const match = newSkillMd.match(/version:\s*([\d.]+)/)
          const newVer = match ? `v${match[1]}` : '最新'
          console.log(`✅ 更新完成！新版本: ${newVer}`)
        } catch {
          console.log(`✅ 更新完成！`)
        }
        break
...
工具:
  version                 查看当前 Skill 版本
  update                  自动更新到最新 Skill
