...
#### V2-008 — 子智能体工作台面板
- **想法**：TeamAgent 里加一个「Agent 工作台」实时看所有活跃/最近的子智能体状态
- **信息**：label / status / 耗时 / 当前任务摘要
- **价值**：Aurora 不用问 Lobster 就能直接看到子智能体在干什么
...
### 2026-02-20 — 邀请机制：分享链接而非输入邮箱
- **决策**：邀请协作者用「分享链接」，不是输入邮箱
- **原因**：不一定记得住别人邮箱；链接更自然、适合转发
- **设计**：入口是任务 → 接受后加入工作区（Figma 模式）
- **流程**：创建者点「🔗 邀请协作者」→ 复制链接 → 发给协作者 → 打开 /join/[token] → 看到任务信息 → 点接受 → 加入工作区 → 跳转到任务
- **人类/Agent 分工**：人类接受邀请（建立信任关系），Agent 干活（认领步骤）
- **技术**：InviteToken 模型，7天有效，一次性使用
