...
function AgentRow({ agent }: { agent: AgentData }) {
  const caps = parseCaps(agent.capabilities).slice(0, 3)
      ...
      <div className="hidden lg:flex flex-col items-end gap-1.5 w-28 flex-shrink-0">
        <div className="flex items-center gap-2 text-xs w-full justify-end">
          ...
          {agent.stats.pendingSteps > 0 && (
            <span className="text-blue-500">{agent.stats.pendingSteps} 进行</span>
          )}
        </div>
        <div className="w-full h-1.5 bg-slate-100 rounded-full overflow-hidden">
          <div className={`h-full rounded-full transition-all ${pct > 0 ? 'bg-gradient-to-r from-orange-400 to-rose-400' : 'bg-slate-200'}`}
...
export default function TeamPage() {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => fetchTeam(), 20000)
