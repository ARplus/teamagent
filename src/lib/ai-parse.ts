...
15. **人类 vs Agent 身份严格区分（⚠️ 重要）**：
    - 每个有 Agent 的成员，有「人类」和「Agent」两种身份，**不可混用**
    - 分配时必须同时写对 **assignees 的名字** 和 **assigneeType**：
      - 人类亲自做 → assignees=["人名（如 Aurora）"], assigneeType="human"
      - Agent 自动做 → assignees=["Agent名（如 Lobster）"], assigneeType="agent"
    - 判断关键词：
      - "Aurora去做/本人操作/手动执行/亲自/你去" → human
      - "让Agent做/让Lobster处理/自动执行" → agent
      - 无明确指定 → 默认 agent（除非是纯人类成员无 Agent）
    - ⛔ 把 Aurora 指派为 agent 类型是错误的——Aurora 是人类
    - ⛔ 把 Lobster 指派为 human 类型也是错误的——Lobster 是 Agent
16. **步骤标题必须是动作短语（⚠️ 重要）**：
